...
export const FAILED_LOGINS: LoginEntry[] = LOGIN_HISTORY.filter((l) => l.status === "failed");

export const IP_WHITELIST: IPEntry[] = [
...
export const IP_BLACKLIST: IPEntry[] = [
...
export const PERMISSION_ACTIONS = ["read", "write", "create", "delete", "manage"];
export const PERMISSION_RESOURCES = ["users", "content", "media", "analytics", "settings", "security", "roles", "audit"];
...
export const AUDIT_LOGS: AuditLog[] = [
  ...
  { id: "a3", user: "system", action: "auth.failed", resource: "login", details: "Failed login attempt from Moscow, RU — incorrect password", ip: "45.33.22.11", timestamp: "5h ago", severity: "warning" },
  ...
  { id: "a5", user: "system", action: "ip.blacklisted", resource: "security", details: "Auto-blacklisted IP 45.33.22.11 — brute force", ip: "45.33.22.11", timestamp: "5h ago", severity: "warning" },
  ...
  { id: "a7", user: "system", action: "backup.completed", resource: "database", details: "Automated backup completed — 2.4 GB", ip: "-", timestamp: "1d ago", severity: "info" },
...
export const SECURITY_ALERTS: SecurityAlert[] = [
  { id: "al1", title: "Brute Force Attack Detected", description: "Multiple failed login attempts from 45.33.22.11 (Moscow, RU) — 12 attempts in 5 minutes", severity: "critical", status: "investigating", timestamp: "5h ago", source: "Rate Limiter" },
  { id: "al2", title: "New Device Login", description: "First login from unknown device in Beijing, CN — 2FA challenge was sent", severity: "high", status: "open", timestamp: "2d ago", source: "Auth Service" },
  { id: "al3", title: "API Rate Limit Warning", description: "/api/auth/login endpoint reached 80% of rate limit threshold", severity: "medium", status: "open", timestamp: "2d ago", source: "Rate Limiter" },
  { id: "al4", title: "SSL Certificate Expiring", description: "Wildcard certificate for *.portfolio.dev expires in 14 days", severity: "medium", status: "open", timestamp: "3d ago", source: "Certificate Manager" },
  { id: "al5", title: "Database Backup Failed", description: "Scheduled backup failed — disk space critical on replica-2", severity: "high", status: "resolved", timestamp: "4d ago", source: "Backup Service" },
  { id: "al6", title: "Suspicious API Key Usage", description: "API key 'ci-cd-key' was used from an unrecognized IP range", severity: "high", status: "resolved", timestamp: "5d ago", source: "API Gateway" },
  { id: "al7", title: "Dependency Vulnerability", description: "CVE-2026-1234 detected in lodash (critical severity)", severity: "critical", status: "resolved", timestamp: "1w ago", source: "Dependency Scanner" },
];
...
export const RECOVERY_CODES: RecoveryCode[] = [
...
export const BACKUP_STATUS = {
  ...
  databases: [
    { name: "PostgreSQL (Production)", size: "1.8 GB", lastBackup: "1h ago", status: "success" as const },
    { name: "PostgreSQL (Staging)", size: "420 MB", lastBackup: "1h ago", status: "success" as const },
    { name: "MongoDB (Analytics)", size: "240 MB", lastBackup: "2h ago", status: "success" as const },
    { name: "Redis (Cache)", size: "12 MB", lastBackup: "1h ago", status: "success" as const },
  ],
};
...
export const SECURITY_SCORE = {
  ...
  findings: [
    { severity: "critical" as const, count: 1, label: "Critical" },
    { severity: "high" as const, count: 3, label: "High" },
    { severity: "medium" as const, count: 5, label: "Medium" },
    { severity: "low" as const, count: 8, label: "Low" },
  ],
};
...
export function getAlertColor(severity: string) {
  return alertColors[severity as keyof typeof alertColors] || alertColors.info;
}
