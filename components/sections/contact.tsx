...
const methodIcons: Record<string, React.ElementType> = {
  email: Mail, github: Github, linkedin: Linkedin, website: Globe, location: MapPin,
};
...
export function Contact() {
  ...
  if (settings?.contactEmail) contactMethods.push({ icon: Mail, label: "Email", value: settings.contactEmail, href: `mailto:${settings.contactEmail}` });
  if (settings?.location) contactMethods.push({ icon: MapPin, label: "Location", value: settings.location });
  socialLinks.forEach((link) => {
    const Icon = methodIcons[link.platform.toLowerCase()];
    if (Icon) contactMethods.push({ icon: Icon, label: link.label, value: link.handle || link.url, href: link.url });
  });
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify(data), signal: abortRef.current.signal,
      });
          ...
          <div className="lg:col-span-3">
            <GlassCard className="p-6 md:p-8">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                ...
                <InputField id="contact-subject" label="Subject" error={errors.subject?.message} placeholder="What's this about?" {...register("subject")} />
