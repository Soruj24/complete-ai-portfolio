...
export default function BlogCategoriesPage() {
  const { data: response, isLoading } = useGetAdminResourceQuery({ resource: "blog-categories" });
  const items: BlogCategory[] = response?.data ?? [];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">Manage blog post categories</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="border-border-subtle bg-surface">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-text-tertiary">No blog categories yet</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4">
          {items.map((item: BlogCategory) => (
