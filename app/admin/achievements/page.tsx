...
export default function AchievementsPage() {
  const { data: response, isLoading } = useGetAdminResourceQuery({ resource: "achievements" });
  const items: Achievement[] = response?.data ?? [];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">Manage your achievements and accomplishments</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="border-border-subtle bg-surface">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-text-tertiary">No achievements yet</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4">
