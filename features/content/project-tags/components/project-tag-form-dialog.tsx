...
interface Props {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<ProjectTag>) => Promise<void>;
}

export function ProjectTagFormDialog({ open, onClose, onSubmit }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ name: name.trim(), slug: name.trim().toLowerCase().replace(/\s+/g, "-"), color, projectCount: 0, createdAt: new Date().toISOString() });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
            ...
            <div className="flex items-center justify-between border-b border-border-primary px-6 py-4">
              <h2 className="text-lg font-semibold text-text-primary">New Tag</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-1.5">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Tag name" required autoFocus
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <button type="submit" disabled={submitting || !name.trim()}
                  ...
                  {submitting ? "Creating..." : "Create Tag"}</button>
              </div>
            </form>
          </motion.div>
        </div>
