...
import { Search, Users, UserCheck, UserX, Shield, MoreHorizontal, Plus } from "lucide-react";
import type { User } from "../types";

const MOCK: User[] = Array.from({ length: 18 }, (_, i) => ({
  id: `usr-${i + 1}`, name: `User ${i + 1}`, email: `user${i + 1}@example.com`,
  role: ["Admin", "Editor", "Viewer", "Manager"][i % 4],
  status: i < 13 ? "active" : i < 16 ? "inactive" : "suspended",
  joinedAt: new Date(2025, i, 1).toISOString(),
  lastLogin: i % 3 !== 0 ? new Date(2026, 6, 1 + i).toISOString() : undefined,
  projects: Math.floor(Math.random() * 12),
}));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          { label: "Total Users", value: MOCK.length, icon: Users, color: "text-accent" },
          { label: "Active", value: MOCK.filter((u) => u.status === "active").length, icon: UserCheck, color: "text-success" },
          { label: "Inactive", value: MOCK.filter((u) => u.status === "inactive").length, icon: UserX, color: "text-warning" },
          { label: "Suspended", value: MOCK.filter((u) => u.status === "suspended").length, icon: Shield, color: "text-error" },
        ].map((s, i) => (
